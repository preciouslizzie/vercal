...
API.createEvent = async (data) => {
  try {
    return await API.post('/admin/event', data);
  } catch (err) {
  ...
  return API.post('/events/create.php', data);
};

API.updateEvent = async (id, data) => {
  try {
    return await API.put(`/admin/event/${id}`, data);
  } catch (err) {
    const status = err?.response?.status;
    if (status && status !== 404 && status !== 405) throw err;
  }
  ...
  return API.post(`/events/update.php?id=${id}`, data);
};
...
API.deleteEvent = async (id) => {
  ...
  return API.delete(`/events/delete.php?id=${id}`);
};
